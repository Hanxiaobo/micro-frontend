...
function App() {
  ...
  const transformArrToTabelArr = (columnList, result) => {
    return result.map((arr) => {
      const res = {}
      // eslint-disable-next-line no-return-assign
      arr.forEach((item, index) => {
        const key = columnList[index]
        res[uniqueKeyFn(res, key)] = item === 'null' ? '-' : item || '-'
      })
      return res
    })
  }
  const uniqueKeyFn = (obj, key) => {
    if (obj[key]) {
      return uniqueKeyFn(obj, key + ' ')
    } else {
      return key
    }
  }
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const pageSize = getQueryVariable('size')
      setSize(pageSize || 1000)

      const res = await queryResult({
        jobId: jobId,
        pageNo: 1,
        pageSize: pageSize * 1 || 1000,
      })
      const data = transformArrToTabelArr(
        res.columnNameList,
        res.resultData || []
      )
      // const ctx = JSON.stringify(data)
      // dataLoader.setUserInput(ctx, { type: 'paste' })
      dataLoader.setJsonData(data, '')
    }
  ...
  return (
    <div className="App">
      ...
      <div className="app-sections">
        <Section
          title={`1. 加载数据 (只获取当前数据集的前${size}行)`}
          loading={loading}
        >
        ...
        {data && currentChart && (
          <Section title={`3. 映射配置`} loading={mappingLoading}>
            <DataMapping
              ref={dataMappingRef}
